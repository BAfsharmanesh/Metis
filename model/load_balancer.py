...
class LayerLoadBalancer:
    ...
    def _get_nomalize_layer_duration(self) -> List[float]:
        ...
        key = f"tp1_bs{self.min_bs}"
        layers_duration = self.profile_data[device_type][key]["time"]["layer-computes"]

        total_layer_duration = sum(layers_duration)
        return [
            layer_duration / total_layer_duration for layer_duration in layers_duration
        ]

    def _get_stage_memory_demand(
        self,
        ...
        for stage_id, strategy in enumerate(strategies):
            ...
            # print(f"{start_rank=}, {end_rank=}, {device_types=}, {device_group=}")
            cur_device_types = [
                device_types[rank] for rank in range(start_rank, end_rank)
            ]

            start_layer_id, end_layer_id = (
                layer_partition[stage_id],
                layer_partition[stage_id + 1],
            )
            cur_stage_memory_demand = 0.001
            if len(set(cur_device_types)) == 1:
                bs = gbs // batches // dp_deg
                profile_memory = self.profile_data[f"DeviceType.{device_types[0]}"][
                    f"tp{tp_deg}_bs{bs}"
                ]["memory"]
                cur_stage_memory_demand += (
                    sum(profile_memory[start_layer_id:end_layer_id]) * mem_coef * dp_deg
                )
            else:
                ...
                for h_mbs in hetero_bs:
                    comb_h_mbs = [
                        2**i
                        for i in range(
                            int(math.log2(h_mbs)) if h_mbs != 0 else 0, -1, -1
                        )
                        if h_mbs & 2**i
                    ]
                    for slice_h_mbs in comb_h_mbs:
                        profile_memory = self.profile_data[
                            f"DeviceType.{device_types[0]}"
                        ][f"tp{tp_deg}_bs{slice_h_mbs}"]["memory"]
                        cur_stage_memory_demand += (
                            sum(profile_memory[start_layer_id:end_layer_id])
                            * mem_coef
                            * dp_deg
                        )
    ...
    def _detect_out_of_memory(
        self, stage_memory_demand: List[float], stage_memory_capacity: List[float]
    ) -> Tuple[bool, List[float]]:
        memory_usage = [
            m_capa - m_demand
            for m_capa, m_demand in zip(stage_memory_capacity, stage_memory_demand)
        ]
    ...
    def _partition_layers_by_compute_performance(
        self, stage_compute_performance: List[float]
    ) -> Tuple[List[int], List[float]]:
        compute_balancer = LayerComputeBalancer(
            len(stage_compute_performance),
    ...
    def _adj_compute_performance(
        self, sc_capa: List[float], sm_capa: List[float], sm_demand: List[float]
        ...
        additional_alloc_sc_capa = [0.0 for _ in range(len(sc_capa))]
        while extra_required_capacity > 0.01:
            tmp_total = sum(
                [
                    c_capa if a_capa > 0.001 else 0
                    for a_capa, c_capa in zip(available_compute_capacity, sc_capa)
                ]
            )
            c_capa_ratio = [
                c_capa / tmp_total if a_capa > 0.001 else 0
                for a_capa, c_capa in zip(available_compute_capacity, sc_capa)
            ]

            for (stage_id, c_ratio), a_capa in zip(
                enumerate(c_capa_ratio), available_compute_capacity
            ):
                alloc_capa = (
                    a_capa
                    if extra_required_capacity * c_ratio > a_capa
                    else extra_required_capacity * c_ratio
                )
        ...
        adj_sc_capa = [
            alloc_capa + adj_capa
            for alloc_capa, adj_capa in zip(additional_alloc_sc_capa, adj_sc_capa)
        ]
        return adj_sc_capa

    def _device_types_by_node_sequence(
        self, node_sequence: List[DeviceType]
    ) -> List[str]:
        device_names = [
            self.gpu_cluster.nodes[node_id].device_type.name
        ...
        num_devices = {}
        for node_id in self.gpu_cluster.nodes.keys():
            key = self.gpu_cluster.nodes[node_id].device_type.name
            if key in num_devices:
                num_devices[key] += self.gpu_cluster.nodes[node_id].num_devices
            else:
                num_devices[key] = self.gpu_cluster.nodes[node_id].num_devices        

        # device_type_count = Counter(device_names)
        # print(f"{num_devices=}, {device_names=}, {device_type_count=}, {node_sequence=}")
        sorted_device_types = []
        for key in node_sequence:
            sorted_device_types.extend([key] * num_devices[key])
    ...
    def partition_layer(
    ...
    ) -> Tuple[Union[List, None], int, Union[List, None]]:
        ...
        while cur_partition_attempt <= max_partition_attempts:
            layer_partition, stage_compute_demand = (
                self._partition_layers_by_compute_performance(stage_compute_performance)
            )
            stage_memory_demand = self._get_stage_memory_demand(
                layer_partition,
                strategies,
                plan.device_groups,
                device_types,
            ...
            memory_exceeded, memory_state = self._detect_out_of_memory(
                stage_memory_demand, stage_memory_capacity
            )
            print(f"layer_partition: {layer_partition}")
            print(
                f"stage_memory_demand: {stage_memory_demand}, memory_state: {memory_state}"
            )
            if not memory_exceeded:
                return layer_partition, cur_partition_attempt, memory_state

            stage_compute_performance = self._adj_compute_performance(
                stage_compute_performance, stage_memory_capacity, stage_memory_demand
            )
            ...
            cur_partition_attempt += 1
            print(
                f"adj_stage_compute_performance({cur_partition_attempt}): {stage_compute_performance}"
            )
        return None, -1, None
...
class DataLoadBalancer:
    ...
    def _get_execution_time(self, device_type: str, key: str) -> float:
        return sum(
            self.profile_data[f"DeviceType.{device_type}"][key]["time"][
                "layer-computes"
            ]
        )

    def partition_data(
        self, device_types: List[str], intra_strategy: Tuple[int, int], bs: int
    ) -> List[int]:
        ...
        inner_stage_compute_performance = [
            s_performance / inner_total_performance
            for s_performance in inner_stage_performance
        ]

        hetero_bs = [
            int(bs * c_performance) for c_performance in inner_stage_compute_performance
        ]
        remainder = bs - sum(hetero_bs)

        remainder_ratio = [
            (bs * c_performance) - int(bs * c_performance)
            for c_performance in inner_stage_compute_performance
        ]

        sorted_indices = sorted(
            range(len(remainder_ratio)), key=lambda i: remainder_ratio[i], reverse=True
        )
...
class LayerComputeBalancer:
    ...
    def _alloc_unassigned_first_pass(self):
        def get_proper_stage(d_layer_group: Dict, q: int) -> int:
            min_stage_id, max_stage_id = min(list(d_layer_group.keys())), max(
                list(d_layer_group.keys())
            )
            ...
            for key in d_layer_group.keys():
                ...
                cur_min_value, cur_max_value = min(list(inner_group)), max(
                    list(inner_group)
                )
    ...
    def _alloc_real_value(self):
        ...
        for stage_id in range(self.num_stage):
            real_value_group = [
                int(layer_id / self.hallucination)
                for layer_id in self.lid_alloc_stage[stage_id]
            ]
            filtered_real_value = [
                layer_id
                for layer_id in real_value_group
                if real_value_group.count(layer_id) > (self.hallucination / 2)
            ]
        ...
        for stage_id in range(len(lid_alloc_stage.keys())):
            if len(lid_alloc_stage[stage_id]):
                s_layer_id, e_layer_id = (
                    lid_alloc_stage[stage_id][0],
                    lid_alloc_stage[stage_id][-1],
                )
                sc_capa.append(
                    self.sc_capa_bak[stage_id]
                    - sum(self.lc_demand[s_layer_id : e_layer_id + 1])
                )
            else:
