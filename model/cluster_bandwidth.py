...
class ClusterBandwidth(ABC):
    def __init__(self, gpu_cluster):
        ...
        self.rank_map, self.rank_node_map = self._get_device_placement(
            num_nodes=gpu_cluster.get_num_nodes(),
    ...
    def _get_device_placement(
        self, num_nodes: int, num_devices: int
    ) -> Tuple[Dict, Dict]:
        ...
        nodes = [
            self.gpu_cluster.nodes[node_id].num_devices
...
class HomoClusterBandwidth(ClusterBandwidth):
    ...
    def _get_model_groups(self, strategy: Tuple[int, int, int]) -> NDArray:
        ...
        assert (
            tp_deg * dp_deg * pp_deg == self.total_devices
    ...
    def get_slowest_pp_bandwidth(
        self, strategy: Tuple[int, int, int], stage_id: int
    ) -> int:
...
class HetClusterBandwidth(ClusterBandwidth):
    ...
    def _get_pp_groups(self, stage_id: int) -> Tuple[List, List]:
        pp_group_ranks = [
            i
            for i in range(
                sum(self.device_groups[:stage_id]),
                sum(self.device_groups[: stage_id + 2]),
            )
        ]
        pp_group_nodes = [
            self.rank_node_map[device_rank] for device_rank in pp_group_ranks
        ]
    ...
    def _get_dp_groups(self, stage_id: int, strategy: Tuple[int, int]) -> List:
        device_ranks = [
            ...
            for i in range(
                sum(self.device_groups[:stage_id]),
                sum(self.device_groups[: stage_id + 1]),
            )
        ]
    ...
    def get_slowest_pp_bandwidth(self, stage_id: int) -> int:
        ...
        device_types = [
            sorted_device_types[node_id] for node_id in list(set(pp_group_nodes))
        ]
    ...
    def get_slowest_dp_bandwidth(self, strategy: Tuple[int, int], stage_id: int) -> int:
        ...
        for dp_group in dp_groups:
            ...
            device_types = [
                sorted_device_types[node_id] for node_id in list(set(group_nodes))
            ]
