...
class GPUCluster:
    def __init__(
        ...
        clusterfile_path: str = None,
        host_entries: Dict = None,
        nodes_info: Dict = None,
    ):
        self.host_entries = host_entries if host_entries else parse_hostfile(hostfile_path)
        self.nodes_info = nodes_info if nodes_info else parse_nodefile(clusterfile_path)

        self.nodes = dict()
        for node_id, node in self.host_entries.items():
            node_ip = node["ip"]
            self.nodes[node_id] = GPUNode(
                device_type=DeviceType.from_string(
                    self.nodes_info[node_ip]["instance_type"]
                ),
    ...
    def get_num_nodes_by_device_type(self, device_type: str) -> int:
        return sum(
            [
                self.nodes[node_id].num_devices
                for node_id in self.nodes.keys()
                if self.nodes[node_id].device_type.name == device_type
            ]
        )
    ...
    def get_total_num_devices(self) -> int:
        num_devices_list = [
            self.nodes[node_id].num_devices for node_id in self.nodes.keys()
        ]
    ...
    def get_str_device_types(self) -> str:
        return "_".join(
            [device_type.name for device_type in set(self.get_device_types())]
        )
